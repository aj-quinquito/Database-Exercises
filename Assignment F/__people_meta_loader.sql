-- ------------------------------------------------------------------
-- COPY CODE to your master boxstore file to the very end
-- just after your people table UPDATEs of the Instructor and Student 
-- p_id IN (1,2) respectively.  Enable the code, by removing the
-- multi-line comments /* (just below) and */ on the last line.
-- Re-add the /* and */ when complete.

-- copy your Explicit SELECT (that lists all columns FROM people p
-- table) to the end of this code, don't cut it out from where it is, copy it to the 
-- just copy to the end of this.
