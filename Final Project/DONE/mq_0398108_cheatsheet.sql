...
-- Upper Case Function, returns: BRAD
...
-- syntax
SELECT SUBSTR(string, pos_beg, length);
...
-- CHAR_LENGTH function, returns 9
...
SELECT SUM(pe_salary) , AVG(pe_salary)
	, MIN(pe_salary) , MAX(pe_salary)
	, COUNT(pe_salary), COUNT(DISTINCT pe_salary)
...
-- GROUP BY
SELECT gco.co_id, gco.co_name, COUNT(gtc.tc_id) AS towncity_cnt
FROM geo_towncity gtc
	JOIN geo_region grg ON gtc.rg_id=grg.rg_id
	JOIN geo_country gco ON grg.co_id=gco.co_id
...
-- -------------------------------------------------------------------
-- INSERT records into other table that don't exists in the other
...
-- just re-run your DROP/CREATE/TRUNCATE/INSERT/SELECT blocks for
-- geo_region and geo_country to properly restore the tables later
-- ...if you delete them, then reinsert, _id's will change from what
-- you'd expect
